...
import { runExitCode, runText } from './_exec';
...
import { executeJestRun } from './jest-execution';
import {
...
import { refineJestSelection } from './refine-related';
...
export const program = async (): Promise<void> => {
  ...
  if (selectionHasPaths && prodSelections.length > 0) {
    const refined = await refineJestSelection({
      selectionHasPaths,
      selectionLooksLikeTest,
    ...
    jestFiles = refined.jestFiles.slice();
    effectiveJestFiles = refined.effectiveJestFiles.slice();
    perProjectFiltered.clear();
    for (const [cfg, owned] of refined.perProjectFiltered.entries()) {
      perProjectFiltered.set(cfg, owned.slice() as string[]);
    }
  }
  ...
  if (shouldRunJest) {
    ...
    const result = await executeJestRun({
      jestBin,
      projectConfigs,
      perProjectFiltered,
      selectionHasPaths,
    ...
    jestExitCode = result.exitCode;
    allBridgeJson.push(...result.allBridgeJson);
    // executedTestFilesSet is used for coverage UI context
    for (const f of result.executedTestFiles) executedTestFilesSet.add(f);
  } else {
  ...
  if (
    allBridgeJson.length > 0 &&
    projectConfigs.filter((cfg) => (perProjectFiltered.get(cfg) ?? []).length > 0).length > 1
  ) {
